...
class WooCommerce_Groupons {
	...
	public static function init() {
		// load the translations now so that the string translated in constants.php is also taken into account
	...
	public static function check_dependencies( $disable = false ) {
		...
		if ( !$woocommerce_is_active ) {
			self::$admin_messages[] = "<div class='error'>" . __( '<em>WooCommerce Group Coupons</em> needs the <a href="http://www.woothemes.com/woocommerce/" target="_blank">WooCommerce</a> plugin. Please install and activate it.', WOO_GROUPONS_PLUGIN_DOMAIN ) . "</div>";
		}
		if ( !$woocommerce_is_active ) {
			if ( $disable ) {
