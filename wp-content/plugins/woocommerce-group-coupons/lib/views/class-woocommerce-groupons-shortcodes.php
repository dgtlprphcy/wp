...
class WooCommerce_Groupons_Shortcodes {
	...
	public static function groupons( $atts, $content = null ) {
		...
		$coupons = array();
		foreach( $_coupons as $coupon ) {
			if ( !key_exists( $coupon->get_id(), $coupons ) ) {
				$coupons[$coupon->get_id()] = $coupon;
			}
		}
		...
		foreach( $coupons as $coupon ) {
			...
			$output .= wp_strip_all_tags( $coupon->get_code() );
			$output .= '</div>';
	...
	public static function check_validity( &$coupon, &$valid, &$error_codes ) {
		...
		if ( $coupon->get_id() ) {
			...
			// Expired
			if ( $expiry_date = $coupon->get_date_expires() ) {
			...
			if ( $coupon->get_minimum_amount() > 0 ) {
				if ( $coupon->get_minimum_amount() > $woocommerce->cart->subtotal ) {
					$valid = false;
			...
			if ( count( $coupon->get_product_ids() ) > 0 ) {
				...
				if ( count( $woocommerce->cart->get_cart() ) > 0 ) {
					foreach( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
						if (
							in_array( $cart_item['product_id'], $coupon->get_product_ids() ) ||
							in_array( $cart_item['variation_id'], $coupon->get_product_ids() ) ||
							in_array( $cart_item['data']->get_parent(), $coupon->get_product_ids() )
						) {
							$valid_for_cart = true;
						}
					}
			...
			if ( $coupon->get_type() != 'fixed_product' && $coupon->get_type() != 'percent_product' ) {
				...
				if ( count( $coupon->get_excluded_product_ids() ) > 0 ) {
					...
					if ( count( $woocommerce->cart->get_cart() ) > 0 ) {
						foreach( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
							if (
								in_array( $cart_item['product_id'], $coupon->get_excluded_product_ids() ) ||
								in_array( $cart_item['variation_id'], $coupon->get_excluded_product_ids() ) ||
								in_array( $cart_item['data']->get_parent(), $coupon->get_excluded_product_ids() )
							) {
								$valid_for_cart = false;
							}
						}
				...
				if ( $coupon->get_exclude_sale_items() == 'yes' ) {
					...
					if ( count( $woocommerce->cart->get_cart() ) > 0 ) {
						foreach( $woocommerce->cart->get_cart() as $cart_item_key => $cart_item ) {
							if (
								in_array( $cart_item['product_id'], $product_ids_on_sale, true ) ||
								in_array( $cart_item['variation_id'], $product_ids_on_sale, true ) ||
								in_array( $cart_item['data']->get_parent(), $product_ids_on_sale, true ) ) {
								$valid_for_cart = false;
							}
						}
					}
	...
	public static function by_code( $a, $b ) {
		return strcmp( $a->get_code(), $b->get_code() );
	}
	...
	public static function get_discount_info( $coupon, $atts = array() ) {
		...
		switch ( $coupon->get_type() ) {
			...
			case 'percent_product' :
				if ( count( $coupon->get_product_ids() ) > 0 ) {
					if ( count( $products ) > 0 ) {
						$result = sprintf( __( '%s%s Discount on %s', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, implode( $product_delimiter, $products ) );
					} else {
						$result = sprintf( __( '%s%s Discount on selected products', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix );
					}
				} else if ( count( $coupon->get_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s Discount in %s', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, implode( $category_delimiter, $categories ) );
				} else if ( count( $coupon->get_excluded_product_ids() ) > 0 || count( $coupon->get_excluded_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s Discount on selected products', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix );
				} else {
					$result = sprintf( __( '%s%s Discount', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix );
				}
				break;
			...
			case 'recurring_percent' :
				...
				if ( count( $coupon->get_product_ids() ) > 0 ) {
					if ( count( $products ) > 0 ) {
						$result = sprintf( __( '%s%s %s on %s', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, $discount_name, implode( $product_delimiter, $products ) );
					} else {
						$result = sprintf( __( '%s%s %s on selected products', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, $discount_name );
					}
				} else if ( count( $coupon->get_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s %s in %s', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, $discount_name, implode( $category_delimiter, $categories ) );
				} else if ( count( $coupon->get_excluded_product_ids() ) > 0 || count( $coupon->get_excluded_product_categories() ) > 0 ) {
					$result = sprintf( __( '%s%s %s on selected products', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, $discount_name );
				} else {
					$result = sprintf( __( '%s%s %s', WOO_GROUPONS_PLUGIN_DOMAIN ), $coupon->get_amount(), $amount_suffix, $discount_name );
				}
				break;
		}
