...
class Referrals_List_Table extends Admin_List_Table {
	...
	function column_customer( $referral ) {
		if ( $user = $referral->get_customer()  ) {
			return '<a href="'. get_edit_profile_url( $user->ID ) .'">' . esc_html( AW_Referrals()->admin->get_formatted_customer_name( $user ) ) . '</a>';
		}
		else {
	...
	function column_created( $referral ) {
		return esc_html( Format::date( $referral->date ) );
	}
	...
	function column_order_id( $referral ) {
		...
		if ( ! $order ) {
			return sprintf( __( '#%s - Deleted', 'automatewoo' ), $referral->get_order_id() );
		}

		return '<a href="'. get_edit_post_link( Compat\Order::get_id( $order ) ) .'">#' . $order->get_order_number(). '</a>'
	...
	function column_offer( $referral ) {
		...
		switch ( $referral->offer_type ) {
			case 'coupon_discount':
				return wc_price( $referral->offer_amount );
				break;

			case 'coupon_percentage_discount':
				return $referral->offer_amount . '% ';
				break;
		}
	...
	function column_status( $referral ) {
		return '<div class="automatewoo-referral-status status-' . esc_attr( $referral->get_status() ) . ' tips" data-tip="'.$referral->get_status_name().'"></div>';
	}
	...
	function column_actions( $referral ) {
		...
		foreach ( $actions as $action => $action_name ) {
			$html .= '<a class="button '.$action.' aw-button-icon tips" data-tip="'.$action_name.'" href="' . Admin_Referrals_Controller::get_route_url( $action, $referral ) .'"></a> ';
		}
	...
	function display_section_nav() {
		...
		foreach ( $this->sections as $id => $label ) {
			$url = add_query_arg( 'section', $id, Admin_Referrals_Controller::get_route_url() );
			echo '<li><a href="' . $url . '" class="' . ( $this->get_current_section() == $id ? 'current' : '' ) . '">' . $label;
			echo isset( $this->section_totals[$id] ) ? ' <span class="count">(' . $this->section_totals[$id] . ')</span>' : '';
