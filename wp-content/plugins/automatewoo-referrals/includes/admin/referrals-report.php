...
class AW_Referrals_Report extends AW_Report_Abstract_Graph {
	...
	function load_chart_data() {
		...
		foreach ( $orders->posts as $order_id ) {
			...
			$order_obj->date = \AutomateWoo\Compat\Order::get_date_created( $order );
					...
					var series = [
						{
							...
							yaxis: 1,
							color: '<?php echo $this->chart_colours['number']; ?>',
							bars: { fillColor: '<?php echo $this->chart_colours['number']; ?>', fill: true, show: true, lineWidth: 0, barWidth: 60 * 60 * 24 * 1000, align: 'center' },
							shadowSize: 0,
							hoverable: false
						},
						{
							...
							yaxis: 2,
							color: '<?php echo $this->chart_colours['value']; ?>',
