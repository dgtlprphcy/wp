...
class Admin {
	...
	function admin_pages( $slug ) {
		add_submenu_page( $slug, __( 'Referrals','automatewoo'), __( 'Referrals','automatewoo'), 'manage_woocommerce', 'automatewoo-referrals', [ 'AutomateWoo\Referrals\Admin_Referrals_Controller', 'output' ] );
		add_submenu_page( $slug, __( 'Referral advocates','automatewoo'), __( 'Advocates','automatewoo'), 'manage_woocommerce', 'automatewoo-referral-advocates', [ 'AutomateWoo\Referrals\Admin_Advocates_Controller', 'output' ] );
		add_submenu_page( $slug, __( 'Referral codes','automatewoo'), __( 'Referral codes','automatewoo'), 'manage_woocommerce', 'automatewoo-referral-codes', [ 'AutomateWoo\Referrals\Admin_Referral_Codes_Controller', 'output' ] );
		add_submenu_page( $slug, __( 'Referral invites','automatewoo'), __( 'Invites','automatewoo'), 'manage_woocommerce', 'automatewoo-referral-invites', [ 'AutomateWoo\Referrals\Admin_Invites_Controller', 'output' ] );
	}
	...
	function enqueue_scripts() {
		...
		wp_register_script( 'automatewoo-referrals-admin', AW_Referrals()->url('/assets/js/automatewoo-referrals-admin' . $suffix . '.js'), ['automatewoo'], AW_Referrals()->version );
	...
	function email_preview_subject( $subject, $type ) {
		...
		return AW_Referrals()->options()->email_share_subject;
	}
	...
	function email_preview_html( $type, $args ) {
		...
		$email = new Invite_Email( $user->user_email, new Advocate( $user ) );
	...
	function email_preview_send_test( $type, $to, $args ) {
		...
		foreach ( $to as $email ) {
			$mailer = new Invite_Email( $email, new Advocate( get_current_user_id() ) );
			$send = $mailer->send();
		...
		switch ( $page ) {
			...
			case 'view-referral':
				return add_query_arg( [
					'referral_id' => $data
					], admin_url( 'admin.php?page=automatewoo-referrals&action=view' ) );
				break;
		}
	}
	...
	function get_formatted_customer_name_from_order( $order ) {
		...
		return AutomateWoo\Compat\Order::get_billing_email( $order );
	}
