...
class Account_Endpoint {
	...
	function endpoint_title( $title ) {
		...
		if ( isset( $wp_query->query_vars[ self::$endpoint ] ) && ! is_admin() && is_main_query() && in_the_loop() && is_account_page() ) {
	...
	function endpoint_content() {
		...
		$data['available_store_credit'] = AW_Referrals()->store_credit->get_available_credit( $user_id );
		$data['used_referrals'] = AW_Referrals()->get_used_referrals_by_user( $user_id );
		$data['advocate'] = new Advocate( $user_id );
