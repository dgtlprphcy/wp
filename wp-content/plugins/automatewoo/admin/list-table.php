...
abstract class Admin_List_Table extends \WP_List_Table {
	...
	protected function get_table_classes() {
		return [ 'automatewoo-list-table', 'widefat', 'fixed', 'striped', $this->_args['plural'] ];
	}
	...
	function output_customer_filter() {
		...
		<?php else: ?>
			...
			<select class="wc-customer-search" style="width:203px;" name="_customer_user"
					  data-placeholder="<?php echo $placeholder; ?>"
					  data-allow_clear="true">
				<?php if ( $user_id ) { echo '<option value="' . $user_id . '"' . selected( true, true, false ) . '>' . wp_kses_post( $user_string ) . '</option>'; } ?>
			</select>
