...
class Admin {
	...
	static function init() {
		...
		add_action( 'current_screen', [ __CLASS__, 'includes' ] );
		...
		add_action( 'admin_footer', [ __CLASS__, 'replace_top_level_menu' ] );
		add_action( 'admin_notices', [ __CLASS__, 'license_notices' ] );
		add_action( 'admin_head', [ __CLASS__, 'menu_highlight' ] );


		add_filter( 'admin_body_class', [ __CLASS__, 'body_class' ] );
		add_filter( 'woocommerce_reports_screen_ids', [ __CLASS__, 'inject_woocommerce_reports_screen_ids' ] );
		add_filter( 'editor_stylesheets', [ __CLASS__, 'add_editor_styles' ] );

		if ( aw_request( 'action' ) === 'automatewoo-settings' ) {
			add_action( 'wp_loaded', [ 'AutomateWoo\Admin', 'save_settings' ] );
		}

		if ( aw_request( 'automatewoo-email-preview-loader' ) ) {
			add_action( 'wp_loaded', [ __CLASS__, 'email_preview_loader' ] );
		}
	}
	...
	static function admin_menu() {
		...
		if ( Licenses::is_active() ) {
			...
			$sub_menu['dashboard'] = [
				...
				'function' => [ 'AutomateWoo\Admin', 'load_controller' ]
			];
