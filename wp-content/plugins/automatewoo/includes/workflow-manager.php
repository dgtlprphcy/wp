...
class Workflow_Manager {
	...
	static function delete_related_data( $workflow_id ) {
		...
		$unsubscribes_query = ( new Unsubscribe_Query() )->where( 'workflow_id', $workflow_id );
		$queue_query = ( new Queue_Query() )->where( 'workflow_id', $workflow_id );

		$data = array_merge( $logs_query->get_results(), $unsubscribes_query->get_results(), $queue_query->get_results() );
