...
class Action_Memberships_Change_Plan extends Action_Memberships_Abstract {
	...
	function run() {
		...
		if ( $check_for_membership ) {
			$this->workflow->add_action_log_note( $this, __( 'Plan could not be changed or created because the user already has a membership using the new plan.', 'automatewoo' ) );
			return;
		}
	...
	function create_membership( $user, $new_plan ) {
		...
		if ( is_wp_error( $membership ) ) {
			$this->workflow->add_action_log_note( $this, $membership->get_error_message() );
		}
		else {
			...
			$this->workflow->add_action_log_note( $this, sprintf(
