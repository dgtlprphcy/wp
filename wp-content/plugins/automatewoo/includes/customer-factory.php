...
class Customer_Factory extends Factory {
	...
	static function get_by_email( $email, $create = true ) {
		...
		if ( Cache::exists( $email, 'customer_email' ) ) {
			return static::get( Cache::get( $email, 'customer_email' ) );
		}
	...
	static function generate_unique_customer_key() {
		...
		$key = strtolower( aw_generate_key(20 ) );
	...
	static function update_cache( $customer ) {
		...
		Cache::set( $customer->get_email(), $customer->get_id(), 'customer_email' );
	}
	...
	static function clean_cache( $customer ) {
		...
		if ( $customer->get_user_id() ) {
			Cache::delete( $customer->get_user_id(), 'customer_user_id' );
		}

		if ( $customer->get_guest_id() ) {
			Cache::delete( $customer->get_guest_id(), 'customer_guest_id' );
		}

		Cache::delete( $customer->get_email(), 'customer_email' );
	}
