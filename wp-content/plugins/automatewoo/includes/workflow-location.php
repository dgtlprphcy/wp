...
class Workflow_Location {
	...
	function get_target_object_type() {
		...
		if ( ! isset( $this->target_object_type ) ) {
			...
			elseif ( $subscription = $this->workflow->get_data_item( 'subscription' ) ) {
				$this->target_object_type = 'subscription';
			}
			elseif ( $user = $this->workflow->get_data_item( 'user' ) ) {
				$this->target_object_type = 'user';
			}
			elseif ( $guest = $this->workflow->get_data_item( 'guest' ) ) {
	...
	function get_target_object() {
		if ( $this->get_target_object_type() ) {
			return $this->workflow->get_data_item( $this->get_target_object_type() );
		}
		return false;
	}
