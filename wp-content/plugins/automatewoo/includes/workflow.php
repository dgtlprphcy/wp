...
class Workflow {
	...
	function maybe_run( $data ) {
		...
		if ( $this->validate_workflow() ) {
			...
			if ( $this->get_timing_type() == 'immediately' ) {
				$this->run();
			}
			else {
				$this->queue();
		...
		switch( $this->get_timing_type() ) {
			...
			case 'datetime':
				...
				if ( $datetime ) {
					...
					// convert to UTC
					$utc_date_string = get_gmt_from_date( $date->format( Format::MYSQL ) );
	...
	function setup( $data = false ) {
		...
		if ( Language::is_multilingual() ) {
			Language::set_current( $this->get_language() );
		}
	...
	function unsubscribe_email( $email ) {
		...
		$unsubscribe->set_workflow_id( $this->get_id() );
	...
	function is_unsubscribed( $email ) {
		...
		$unsubscribed = $query->has_results();
	...
	function is_conversion_tracking_enabled() {
		return $this->get_option( 'conversion_tracking' );
	}
