...
class Queue_Manager {
	...
	static function get_batch_size() {
		return (int) apply_filters( 'automatewoo_queue_batch_size', AW()->options()->queue_batch_size );
	}
	...
	static function get_failure_message( $code ) {
		...
		$messages = [
			...
			Queued_Event::F_MISSING_DATA => __( 'Some of the required data was not found. For example, if the customer was deleted.', 'automatewoo' ),
	...
	static function check_for_queued_events() {
		...
		// IMPORTANT - if the previous process is still running cancel it before starting a new one
