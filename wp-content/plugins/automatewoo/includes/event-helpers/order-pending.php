...
class Order_Pending {
	...
	static function schedule_pending_check( $order_id ) {
		...
		wp_schedule_single_event( time() + $delay, 'automatewoo_check_for_pending_order', [ (int) $order_id ] );
	}
	...
	static function maybe_clear_scheduled_check( $order_id, $old_status, $new_status ) {
		if ( $old_status === 'pending' ) {
			wp_clear_scheduled_hook( 'automatewoo_check_for_pending_order', [ (int) $order_id ] ); // must be int
		}
	}
