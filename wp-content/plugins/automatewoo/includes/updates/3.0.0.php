			...
			foreach ( $actions as &$action ) {
				foreach ( $action as $field_name => &$field_value ) {
					...
					$replacer = new Replace_Helper( $field_value, function( $value ) {
						...
						$data_type_and_field = Variables_Processor::parse_data_type_and_field( $value );
						...
						$value = aw_str_replace_start( $value, 'user.', 'customer.' );

						if ( $data_type == 'user' && array_key_exists( $data_value, $user_variables_to_convert ) ) {
							$value = aw_str_replace_start( $value, ".$data_value", ".$user_variables_to_convert[$data_value]" );
						}
